...
if [[ $1 ]]
  ...
  ELEMENT=$($PSQL "SELECT atomic_number, atomic_mass, melting_point_celsius, boiling_point_celsius, symbol, name, type FROM properties JOIN elements USING(atomic_number) JOIN types USING(type_id) WHERE elements.atomic_number = $1")
    ...
    then 
      ...
      echo $ELEMENT | while IFS=\| read ATOMIC_NUMBER ATOMIC_MASS MELTP BOILP SYM NAME TYP
      do
        echo "The element with atomic number $ATOMIC_NUMBER is $NAME ($SYM). It's a $TYP, with a mass of $ATOMIC_MASS amu. $NAME has a melting point of $MELTP celsius and a boiling point of $BOILP celsius."
      done
    fi
  else
